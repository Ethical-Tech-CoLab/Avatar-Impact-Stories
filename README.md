...
someone. There were four separate causes, all now fixed:
...
5. **Cache warming starved the story being watched.** Browsers allow only a handful of
   ...
   the tile appeared to do nothing. Warming is now suspended for as long as a story is on
   screen and resumes when the wall goes idle.

`sw.js` fixes the last two by keeping media in the Cache API, so a story plays from local disk
once fetched. While the wall sits idle it warms the cache in wall order, one file at a time.
The page sends the worker `pause` when a story opens and `resume` when it closes; the warm
queue survives the pause and picks up where it left off, so warming can never compete with a
story someone is actually watching. Media is served with a week-long `Cache-Control`, and the
...
`tools/compress_media.py` re-encodes everything at a constant quality target (x264 CRF 18) rather
than a fixed bitrate, so each video gets exactly the bitrate its content needs. Quality was
verified with **VMAF** against the original files — the same metric Netflix uses — where ≥ 95 is
the accepted threshold for *visually indistinguishable*:
...
- **Bitrate only — no downscaling, no frame-rate changes.** Resolution and frame rate are left
  exactly as the source. A downscale test on `arabic.mp4` was tried and rejected: its VMAF score
  was unreliable (a 1%-low of 0.00 indicates frame desync corrupting the comparison rather than a
  genuine result), and unverifiable savings aren't worth taking on quality-sensitive material.
...
Partly, and it's worth being precise about why. Videos use `preload="none"` and only one plays at
a time, so **total** library size was never what a visitor waited on — the per-video bitrate was.
That is exactly what dropped:

- **Time-to-first-frame** improves a lot. The chocolate story needed ~9.5 Mbps sustained to play
  without stalling; it now needs ~2.6. On a venue's shared Wi-Fi that is the difference between
  smooth playback and rebuffering mid-testimony.
- **Total library size** matters for the *published* build, not the kiosk: it decides whether the
  media fits inside GitHub Pages' 1 GB cap and how much bandwidth the demo burns.
- **Locally it changes little** — off a local disk the originals already played fine. The win is
  for anyone cloning the repo or watching the hosted demo.
