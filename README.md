...
**How it was made: <https://ethical-tech-colab.github.io/Avatar-Impact-Stories/about.html>** —
the provenance of the stories, the tooling and its date, and the ethical boundaries. Linked from
the wall itself, because that is the question people ask once they have watched one.
...
> This section is the maintainer's summary. The visitor-facing version is
> **[`about.html`](about.html)** ([live](https://ethical-tech-colab.github.io/Avatar-Impact-Stories/about.html)),
> which is linked from the start screen and from the corner of the idle wall. Keep the two in
...
short silent loop that lives on the idle wall, and the full MP4 with audio that plays when a
visitor taps it.

**The date is part of the artifact.** Avatar generation is moving fast enough that a few months
reads as a visible generation gap: lip-sync drifts on harder consonants, movement is limited,
gaze is fixed, and emotional range is narrow next to what newer models produce. Those artefacts
are deliberately left in. They record when the work was done, and they help a viewer tell that
the presenter is synthetic. Anyone re-cutting these stories with newer tooling should say so
rather than quietly refreshing them in place.
...
The wall is built for a room, not a browser tab. Phone-shaped tiles loop silently over a crowd
...
| `about.html` | Visitor-facing provenance page: where the stories came from, the tooling and its date, the ethics. Linked from the wall. |
...
| `.github/scripts/check_site.py` | Pre-publish check: every story's media and every page link must resolve. |
